...
export function RevenueByTypeSlides() {
  ...
  const [globalData, setGlobalData] = useState<RevenueData>({ labels: [], data: [], total: 0 });
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const q = query(interventionsRef, where('userId', '==', user.uid), where('accountId', '==', currentAccount.id));
        ...
        querySnapshot.forEach((doc) => {
          ...
          if (intervention.selectedTypes && intervention.date) {
            ...
            intervention.selectedTypes.forEach((type: { name: string; price: number }) => {
              // Global
              const currentGlobal = globalRevenue.get(type.name) || 0;
              globalRevenue.set(type.name, currentGlobal + type.price);

              // Monthly
              if (interventionDate >= startMonth && interventionDate <= endMonth) {
                const currentMonthly = monthlyRevenue.get(type.name) || 0;
                monthlyRevenue.set(type.name, currentMonthly + type.price);
              }

              // Weekly
              if (interventionDate >= startWeek && interventionDate <= endWeek) {
                const currentWeekly = weeklyRevenue.get(type.name) || 0;
                weeklyRevenue.set(type.name, currentWeekly + type.price);
              }
            });
          }
        });
        ...
        // Process global data
        const sortedGlobal = Array.from(globalRevenue.entries()).sort((a, b) => b[1] - a[1]);
        const globalTotal = sortedGlobal.reduce((sum, [, revenue]) => sum + revenue, 0);
        setGlobalData({
          labels: sortedGlobal.map(([name]) => name),
          data: sortedGlobal.map(([, revenue]) => revenue),
          total: globalTotal
        });
  ...
  return (
    <div 
      className="bg-white dark:bg-gray-800 rounded-xl shadow-sm overflow-hidden h-[500px] relative select-none"
